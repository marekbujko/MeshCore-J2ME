...
public class MonochromeImage
{
	...
	/** If true, invert black/white (for inverted QR codes). */
	...
	/** Set inverted mode for white-on-black QR codes. */
	...
	{
		if (0 <= x && x < w && 0 <= y && y < h) {
			boolean black = pixels[x + y * w] <= threshold;
	...
	public boolean get(int[] p)
	{
		if (0 <= p[0] && p[0] < w && 0 <= p[1] && p[1] < h) {
			boolean black = pixels[p[0] + p[1] * w] <= threshold;
			return inverted ? !black : black;
		}
		return false;
	}
